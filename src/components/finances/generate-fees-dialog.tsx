...
export function GenerateFeesDialog({
  ...
  const hasWarning = preview.membersWithoutType > 0

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Beiträge für {preview.year} generieren</DialogTitle>
          <DialogDescription>
        ...
        <DialogFooter>
          ...
          <Button
            onClick={onGenerate}
            disabled={isLoading || isGenerating || (preview.individualCount === 0 && preview.familyCount === 0)}
          >
            {isGenerating ? (
              <>
